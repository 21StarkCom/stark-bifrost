...
description: 'Adversarial red-team review of a design doc. 5 personas (security-trust, reliability-distsys, data, product-dx, cost-ops) attack the design and emit blocking concerns + counter-proposals. Default model: gpt-5.5-pro. Single round, challenge-first; proposed fix plan is generated by default.'
...
- `--fold` — after a successful challenge, fold the proposed fix plan back into
  the design doc by shelling out to `/stark-red-team-fold` (`red_team_fold.ts`):
...
pushes the design doc + sidecar** (unless `--dry-run` or `--no-pr-comment`), so
...
7. If `--fold` and the challenge wrote a sidecar, shells out to `red_team_fold.ts`
   to fold the fix plan into the design doc (non-fatal; under `--json` fold's
...
if none exists, this skill **opens one** and pushes the design doc + sidecar so
...
if [ "$cur" = "$default_branch" ] || [ -z "$cur" ]; then
  stem=$(basename -- "$design_path" | sed -E 's/\.[^.]+$//')
  branch="red-team/design-${stem}-$(date +%Y%m%d-%H%M%S)"
  git switch -c "$branch"
else
  branch="$cur"
fi
...
design. When opening a PR the design doc must also be on the branch ("push the
...
if [ "$open_pr" = 1 ]; then
  git add -- "$design_path"          # opening a PR → land the doc on the branch
  commit_paths=("$design_path" "$sidecar_path")
else
...
- **Doc-not-committed guard:** when we're *not* committing the doc (`open_pr` is
  0) and the design doc has uncommitted changes (`git diff --quiet -- "$design_path"`
  non-zero, or it shows in `git status --porcelain`), still commit the sidecar
  but warn that the findings reference a working-tree version of the design not
  yet in history; the user can commit the design and re-run. (When opening a PR
...
if [ "$open_pr" = 1 ]; then
  ...
  created=$(node --experimental-strip-types "$TOOLS/github_app.ts" --app "$RT_APP" \
      ...
      --head "$branch" \
      --base "$default_branch" \
      --title "Red-team: $(basename -- "$design_path")" \
      --body "Adversarial red-team review of \`$design_path\` ($rt_model). Findings posted as a PR comment by \`$RT_APP\`; full sidecar at \`$sidecar_path\`.")
