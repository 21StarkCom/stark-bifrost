...
`stark-bifrost` (branded **Stark-Bifröst**; GitHub slug `21StarkCom/stark-bifrost`) is the **canonical, multi-runtime marketplace** for stark bundles. The engine renders each bundle into per-runtime trees (`dist/claude/`, `dist/codex/`, `dist/gemini/`) plus a signed `index.json` / `bundles/*.json` web registry. The repo doubles as a native **Claude Code marketplace** — `.claude-plugin/marketplace.json` at the repo root IS the manifest CC reads when you `/plugin marketplace add 21StarkCom/stark-bifrost`.
...
- **Go 1.24**, module `github.com/21StarkCom/stark-bifrost/engine`. Standard layout (`cmd/`, `internal/`), small packages.
