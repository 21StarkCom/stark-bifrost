...
**GitHub Actions are DISABLED (cost control — this is a free public repo, but all GHA is off to guarantee $0 GitHub spend).** `ci`, `sign-manifest`, `web-deploy` are `disabled_manually`; CodeQL default setup + Dependabot version/security updates are off; `main` branch protection has **no required status checks**. **You are the gate now — run `docs/scripts/ci-local.sh` before every push/merge** (it mirrors the old `ci.yml`: validate → drift `build --check` → check-bumps → lint → tests → gitleaks → actionlint). Publish stark-skills changes with `docs/scripts/publish.sh` (`--add-skill NAME --bundle B` / `--remove-skill NAME --bundle B`) — it runs the sync→build→check regen and applies the **semver policy automatically**: a skill add/remove is a **MINOR** bump of that bundle; any other content change is a **PATCH** bump of each affected bundle; the root `VERSION` bumps every publish (MINOR if membership changed this run, else PATCH). Deploy the web origin on-demand via `docs/scripts/deploy-web.sh` (cross-builds a `linux/amd64` image via buildx). Re-enable any workflow with `gh workflow enable <name>` and restore required checks if you reverse this. The `.github/workflows/*.yml` files remain in-tree (disabled by state, not deleted) as the reference for what the local scripts mirror.
